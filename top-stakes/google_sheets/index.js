...
async function writeStakes(auth) {
    ...
    const data = xlsx.parse(path.join(__dirname, '..', 'mp_manager', 'files', 'gathered_stakes.xlsx'))[0]['data'];
    //console.log(data)
...
async function getStakes(auth) {
    const sheets = google.sheets({ version: 'v4', auth });
    const stakes = (await sheets.spreadsheets.values.get({
        spreadsheetId: '1Yw4ubonsE7E1YX2rLxEfjccCqnK0BwD_6P6mH2SCysU',
        range: `Terms!A:F`,
    })).data.values;
    const filteredStakes = stakes.filter(stake => stake.length === 1);
    fs.writeFile(path.join(__dirname, '..', 'mp_manager', 'files', 'current_google_sheets_data.xlsx'), xlsx.build([{ name: 'Terms', data: filteredStakes }]));
    console.log('Synced.');
}
